...
output "iot_indexing_configurations_thing_group_indexing_configuration" {
  ...
  value       = { for k, v in aws_iot_indexing_configuration.iot_indexing_configurations : k => one(v.thing_group_indexing_configuration) if v.thing_group_indexing_configuration != null && length(v.thing_group_indexing_configuration) > 0 }
}
output "iot_indexing_configurations_thing_indexing_configuration" {
  ...
  value       = { for k, v in aws_iot_indexing_configuration.iot_indexing_configurations : k => one(v.thing_indexing_configuration) if v.thing_indexing_configuration != null && length(v.thing_indexing_configuration) > 0 }
}
